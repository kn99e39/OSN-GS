...
@runtime_checkable
class SurfaceOrientationEvidence(Protocol):
    """Structural contract this module actually needs from an orientation source.

    Deliberately NOT `GaussianSurfaceOrientation` -- this partition is
    primitive-agnostic by design (arch/2dgs-coverage-first-surface): it reads
    only `positions`, `surface_normal`, and `gaussian_ids`, so it accepts
    3DGS's eigen-decomposition-derived `GaussianSurfaceOrientation` AND 2DGS's
    intrinsic, decomposition-free `SurfelSurfaceOrientation`
    (`osn_gs/surface/torch_surfel_surface_orientation.py`) without importing
    either concrete type. `unsigned_normal_alignment` above is the one
    geometry helper actually reused across both -- it only compares two given
    unit normals and never derives one, so reusing it does not reintroduce a
    covariance-eigenvector normal definition into this module.
    """
