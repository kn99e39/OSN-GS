...
def diagnose_dense_boundary_connectivity(candidates: Sequence[Any]) -> dict[str, Any]:
    ...
    for i in range(n):
      for sign in (-1,1):
        ...
        for j in range(n):
          ...
          if float(delta.norm())<=2.5*scale: pool.append((float(delta.norm()),j))
        ...
        if len(pool)>1 and abs(pool[1][0]-pool[0][0])<=.1*scale: outcomes['ambiguous_competition']=outcomes.get('ambiguous_competition',0)+1; continue
