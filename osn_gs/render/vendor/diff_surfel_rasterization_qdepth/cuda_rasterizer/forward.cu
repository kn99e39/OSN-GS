...
renderCUDA(
	...
	// OSN-GS DIAGNOSTIC ADDITION (worklog 120, Candidate D) -- see forward.h.
	const float* __restrict__ query_depths,
	...
	float* __restrict__ out_post_median_depth_stats)
{
	...
	// OSN-GS DIAGNOSTIC ADDITION (worklog 120, Candidate D): this pixel's
	// arbitrary-depth query probes, loaded once into registers. q_active
	// counts slots still awaiting resolution, so a pixel with no probe pays
	// exactly one predictable branch per contributor and nothing else. None
	// of these registers is ever read by a canonical computation.
	float q_depth[OSN_GS_MAX_QUERY_SLOTS];
	bool q_pending[OSN_GS_MAX_QUERY_SLOTS];
	int q_active = 0;
	if (inside && query_depths != nullptr)
	{
		for (int q = 0; q < OSN_GS_MAX_QUERY_SLOTS; q++)
		{
			float qd = query_depths[pix_id * OSN_GS_MAX_QUERY_SLOTS + q];
			q_depth[q] = qd;
			q_pending[q] = (qd > 0.0f);
			if (q_pending[q]) q_active++;
		}
	}
	else
	{
	...
	// needed. -1 (never set) means no contributor crossed T=0.5 at this pixel.
	int median_surfel_id = -1;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (test_T < 0.0001f)
			{
				...
				// OSN-GS DIAGNOSTIC ADDITION (worklog 120, Candidate D): this
				// contributor has passed every canonical acceptance check EXCEPT
				// the termination test, which the kernel's own unmodified
				// `T * (1 - alpha) < 0.0001` has just failed. It is therefore a
				// real geometric event at camera-space depth `depth`, and:
				//   * a probe at or in front of `depth` HAS been reached by
				//     canonical traversal (T is still the pre-update value);
				//   * every probe behind `depth` never will be -- traversal
				//     terminates here.
				if (q_active > 0)
				{
					for (int q = 0; q < OSN_GS_MAX_QUERY_SLOTS; q++)
					{
						if (q_pending[q])
						{
							int slot = pix_id * OSN_GS_MAX_QUERY_SLOTS + q;
							bool reached_here = (depth >= q_depth[q]);
							out_query_T[slot] = T;
			...
			// OSN-GS DIAGNOSTIC ADDITION (worklog 120, Candidate D): this
			// contributor is ACCEPTED by the canonical kernel -- it passed
			// depth >= near, power <= 0, alpha >= 1/255 and the termination
			// test -- so it is a real composited event at camera-space depth
			// `depth`, and `T` still holds exactly the transmittance
			// accumulated from every previously accepted contributor (the
			// `T = test_T` update is further below). Any probe at or in front
			// of this depth has therefore been reached by canonical traversal,
			// with nothing having terminated it on the way.
			//
			// Resolution deliberately happens ONLY at accepted contributors.
			// The kernel computes `depth` for every tile-list candidate from
			...
			if (q_active > 0)
			{
				for (int q = 0; q < OSN_GS_MAX_QUERY_SLOTS; q++)
				{
					if (q_pending[q] && depth >= q_depth[q])
					{
						int slot = pix_id * OSN_GS_MAX_QUERY_SLOTS + q;
						out_query_T[slot] = T;
			...
			// Provenance only -- how many accepted contributors precede a probe.
...
#endif
		// OSN-GS DIAGNOSTIC ADDITION (worklog 120, Candidate D): probes that
		// were neither reached nor cut off by the canonical termination --
		// this pixel simply ran out of contributors before the query depth.
		// Recorded as reached=0, terminated=0: nothing ever blocked the ray.
		for (int q = 0; q < OSN_GS_MAX_QUERY_SLOTS; q++)
		{
			if (q_pending[q])
			{
