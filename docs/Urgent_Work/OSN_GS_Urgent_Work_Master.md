...
미착수로 남은 방향: (a) opacity-reset 직후 screen-size prune storm 원인 추적, (b) worklog 69~71이 세 가지 서로 다른 접근(고정 wedge densify, evidence-topology 직접 복원)에서 공통으로 확인한 "typed boundary evidence 자체가 본질적으로 희소·국소적"이라는 근본 한계에 대한 대응(예: half-edge candidate 생성 자체의 밀도를 높이는 방법, 또는 boundary가 아닌 전혀 다른 표현(예: implicit/point-based)으로 전환하는 방법 검토 — 지금까지 시도한 "기존 boundary를 확장/재구성"하는 접근으로는 반복적으로 한계에 부딪혔다는 것이 세 라운드에 걸쳐 확인됨).

## 7. 현재 검증 상태와 알려진 위험

Repository-wide pytest 최신 기준(worklog 64): `783 passed, 1 skipped, 1 warning, 18 subtests passed`. worklog 65는 코드 변경 없는 replay라 재실행 안 함. **worklog 66/67/68/69/70/71은 production 코드를 변경했지만(각각 `torch_density_preserving_representative_selection.py` 순수 성능 최적화, `torch_pipeline.py`+신규 `torch_region_owned_full_evidence.py` additive-only 기능 추가, 신규 `torch_local_orientation_folding.py` 단독 모듈, 신규 `torch_single_chart_uv_validity.py` 단독 모듈, 신규 `torch_region_owned_boundary_materialization.py` 단독 모듈, 신규 `torch_region_owned_full_evidence_boundary_topology.py` 단독 모듈) 지시에 따라 매 라운드 focused pytest만 실행했다(66: 86개, 67: 232개, 68: 5개, 69: 13개, 70: 7개, 71: 14개, 전부 통과) — full-suite 숫자는 여전히 worklog 64 시점 기준이다. 다음 라운드에서 production 코드를 추가로 건드리기 전에 먼저 전체 pytest를 한 번 재확인한다.** §2.3의 negative-control 표와 함께 매 라운드 재확인한다.
...
다음 작업자는 먼저 이 문서와 `docs/worklogs/README.md`의 최신 인덱스, 그리고 가장 최근 작업로그(현재 `docs/worklogs/71_region_owned_full_evidence_boundary_topology_reconstruction.md`)를 읽고 이어서 작업한다. 과거 방향의 세부 기록은 필요할 때 Git history로만 조회한다.
