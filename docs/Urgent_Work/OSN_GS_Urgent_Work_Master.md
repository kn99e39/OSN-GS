...
미착수로 남은 방향: (a) opacity-reset 직후 screen-size prune storm 원인 추적, (b) **worklog 75가 normal source를, worklog 76이 scale domain을 각각 병목에서 확정적으로 제거했으므로**, 남은 후보는 boundary-support predicate 자체(어떤 점을 boundary support로 인정하는가)와 그 evidence 밀도, 또는 boundary-loop이 아닌 다른 표현으로의 전환이다. orientation 개선·scale 재조정·기존 boundary 확장/재구성은 worklog 69~76에 걸쳐 반복적으로 한계가 확인됐으므로 새 근거 없이 재개하지 않는다.

## 7. 현재 검증 상태와 알려진 위험

Repository-wide pytest 최신 기준(worklog 64): `783 passed, 1 skipped, 1 warning, 18 subtests passed`. worklog 65는 코드 변경 없는 replay라 재실행 안 함. **worklog 66~76은 production 코드를 변경했지만(각각 `torch_density_preserving_representative_selection.py` 순수 성능 최적화, `torch_pipeline.py`+신규 `torch_region_owned_full_evidence.py` additive-only 기능 추가, 신규 `torch_local_orientation_folding.py` 단독 모듈, 신규 `torch_single_chart_uv_validity.py` 단독 모듈, 신규 `torch_region_owned_boundary_materialization.py` 단독 모듈, 신규 `torch_region_owned_full_evidence_boundary_topology.py` 단독 모듈, 72~74의 dense-boundary support/진단 3개 단독 모듈, 75의 실험 전용 `torch_structural_normal.py` 단독 모듈, 76의 신규 `torch_boundary_support_spacing.py` + dense-boundary 2개 모듈에 대한 가산 인자) 지시에 따라 매 라운드 focused pytest만 실행했다(66: 86개, 67: 232개, 68: 5개, 69: 13개, 70: 7개, 71: 14개, 72: 16개, 73: 3개, 74: 2개, 75: 15개, 76: 42개, 전부 통과) — full-suite 숫자는 여전히 worklog 64 시점 기준이다. 다음 라운드에서 production 코드를 추가로 건드리기 전에 먼저 전체 pytest를 한 번 재확인한다.** §2.3의 negative-control 표와 함께 매 라운드 재확인한다.
...
다음 작업자는 먼저 이 문서와 `docs/worklogs/README.md`의 최신 인덱스, 그리고 가장 최근 작업로그(현재 `docs/worklogs/76_boundary_support_spacing_scale_domain_decision.md`)를 읽고 이어서 작업한다. 과거 방향의 세부 기록은 필요할 때 Git history로만 조회한다.
