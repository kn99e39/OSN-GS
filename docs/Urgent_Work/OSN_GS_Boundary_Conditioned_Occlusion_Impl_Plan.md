...
상태: **ACTIVE — Phase A–E 구현·검증 완료, Gate B/C/D 승인 완료, Gate E 보고 완료. Phase F 이후 미승인.**
...
상태: **구현·검증 완료, 승인 게이트 D 보고 완료.** 근거: `docs/Urgent_Work/OSN_GS_Phase_D_Continuation_Domain_Design.md`(design revision 3), `docs/worklogs/80_phase_d_continuation_domain_design.md`(설계 이력), `docs/worklogs/81_phase_d_continuation_domain_implementation.md`(구현·Gate D 보고). 아래 작업/검증 항목은 설계 문서의 요약이다 — 상세 수식·자료구조·fixture별 invariant는 설계 문서를, 실제 수치는 worklog 81을 따른다.

Phase D는 최종 parametric surface나 NURBS chart를 만들지 않는다. 역할은 `Visible NURBS boundary → boundary-local world-space sampled continuation strip → geometric validity 및 uncertainty metadata`로 제한된다(설계 문서 §1).

### 작업

1. Boundary world tangent를 world-arclength 기준 유한차분으로 계산한다(설계 문서 §4.1) — `s`는 cumulative world arclength로 단위를 고정하고, sample index/normalized parameter와 혼용하지 않는다. **Closed boundary는 `boundary_length`(closing segment 포함 전체 perimeter) 필드를 별도로 두고, periodic 차분의 wrap-around denominator에 사용한다** — `s_world` 자체는 closing segment를 포함하지 않는다(revision 3).
2. Outward 방향은 UV-space perpendicular가 아니라 **world-space에서 직접** 정의한다: `N = normalize(S_u x S_v)`, `C = normalize(N x T)`, `inner_world` 방향과의 내적으로 부호 선택(설계 문서 §4.2). UV parameterization의 skew/scale에 영향받지 않는다.
3. First-order sampled grid를 canonical baseline이자 유일한 position 표현으로 구현한다(설계 문서 §4.4). Second-order position variant는 제공하지 않는다.
4. Second derivative는 UV로 투영한 뒤(설계 문서 §4.3) **`second_order_displacement_at_extent`/`second_order_growth_ratio`라는 명칭의 diagnostic에만 사용**하고 canonical position에는 반영하지 않는다 — intrinsic mean/Gaussian/normal/geodesic curvature 추정치가 아님을 docstring에 명시한다(revision 3, 이전 `curvature_growth_ratio` 명칭 폐기).
5. `inner_probe_distance`(부호 선택용)와 `local_surface_scale`(`L_boundary`/`L_inner`/`L_control` 중 finite·positive 값의 median, 최소 2개 이상 필요 — 설계 문서 §4.5.1의 canonical 공식으로 고정)과 `continuation_extent`(`= extent_multiplier * local_surface_scale`)를 분리한다 — 하나의 probe distance를 extent로 직접 재사용하지 않는다.
6. Jacobian/orientation/degeneracy를 mask(`normal_valid_mask`/`direction_valid_mask`/`sample_valid_mask`)로 표현한다 — NaN을 쓰지 않는다(설계 문서 §5).
7. **(설계에서 식별한 prerequisite)** `torch_annulus_chart.py`의 Jacobian singular-value 계산과 orientation-consistency 계산을 **두 개의 별도** surface-agnostic 헬퍼(`compute_parametric_jacobian_metrics`, `compute_orientation_consistency`)로 분리 추출하는 순수 리팩터를 Phase D 구현 착수 전에 완료해야 한다(설계 문서 §9). 회귀 검증은 byte-identical이 아니라 상태 분류/report field/수치 tolerance 동일 여부로 확인한다.
8. 입력 오류와 build 실패와 사후 품질 문제를 3단계로 분리한다(설계 문서 §2.2-2.4, revision 3): `reconciled_internal` boundary/patch_id 불일치/shape 불일치/최소 sample 수 미달/인접 duplicate·zero-length segment/명시적 scale·multiplier가 non-finite 또는 `<=0` → **`ValueError`** 즉시 발생. `local_surface_scale` 자동 도출 실패(유효 후보 2개 미만) 등 grid 자체를 만들 수 없는 경우 → **`ContinuationDomainBuildError`**(신규 예외 타입, `ContinuationDomain` 객체를 반환하지 않음). Grid 구성 후의 부분적 품질 문제(Jacobian collapse 등) → `state=degenerate`/`rejected`인 `ContinuationDomain` 반환.
9. 상태 이름은 `valid`/`degenerate`/`rejected`를 쓴다 — Phase E의 `candidate`와 혼동되는 이름은 쓰지 않는다(설계 문서 §5).
...
- (설계 문서 §7에 fixture 전체와 실패 조건 명시)

### Phase D 범위 밖으로 명시적으로 미룬 것(설계 문서 §9)
...
- Phase C `ObservationEvidence`와의 실제 결합 → interface만 정의(설계 문서 §3, `ContinuationDomain.world.reshape(-1,3)`가 `classify_world_samples` 입력과 이미 일치), 호출 없음.
...
상세 계약은 `OSN_GS_Phase_E_Bounded_Candidate_Design.md`. 아래는 canonical 요약이며 상세 문서와 충돌하면 상세 문서를 따른다.
...
| Phase D | 완료, Gate D 승인 | `docs/Urgent_Work/OSN_GS_Phase_D_Continuation_Domain_Design.md`(design revision 3), `docs/worklogs/80_phase_d_continuation_domain_design.md`(설계 이력), `docs/worklogs/81_phase_d_continuation_domain_implementation.md`(구현·Gate D 최종 기록) |
| Phase E | 구현·검증 완료, Gate E 보고 | `docs/Urgent_Work/OSN_GS_Phase_E_Bounded_Candidate_Design.md`(상세 설계), `docs/worklogs/82_phase_e_bounded_candidate_implementation.md`(구현·Gate E 보고) |
...
Phase E 구현·검증을 완료했다(`torch_aabb_broad_phase.py`, `torch_occluded_region_candidate.py`, `torch_candidate_evidence.py`, tests, `worklogs/82`). 다음 승인 요청은 **Phase F — Minimal Constrained Occluded NURBS Bridge**로 한정한다. Phase F, global candidate selection/ranking, multi-sided aggregation, production integration은 별도 승인 전까지 시작하지 않는다.