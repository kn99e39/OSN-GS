...
고정 색상은 OBSERVED=(0.10,0.85,0.35) green, OCCLUDED=(0.92,0.18,0.18) red, UNRESOLVED=(0.60,0.60,0.62) gray다. Occluded Gaussian/volumetric representation이 실제로 없으면 marker를 발명해 표시하지 않으며, validated Occluded volumetric render가 승인된 경우에만 고정 두 결과와 함께 추가한다. 이전 WL123 EVENT_IDENTITY_EFFECT 출력은 이 contract 이전의 historical diagnostic output으로 보존하지만 canonical 비교 결과로 사용하지 않는다. 상세 계약은 [Worklog 125](worklogs/125_fixed_gaussian_visualization_contract.md)와 [architecture decision](architecture.md#fixed-gaussian-visualization-contract) 문서에 기록했다.