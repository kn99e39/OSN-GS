...
## 2026-09-02 Worklog 151 Renderer Evidence / Canonical Ownership 호환성 감사

- [Worklog 151](worklogs/151_renderer_event_canonical_surface_compatibility_audit.md)은 WL150 bypass 이후, renderer median event가 기존 Local Surface Decomposition / Boundary First 계약에 새 의미 없이 들어갈 수 있는지 canonical source와 frozen WL149 baseline으로 검사했다.
...
## 2026-09-04 Worklog 163 Renderer Contributor Provenance 및 Median-Event Observation-Semantics Attribution Audit

- [Worklog 163](worklogs/163_renderer_contributor_provenance_median_event_observation_semantics_attribution_audit.md)는 W162 provenance gap을 기존 isolated diff_surfel_rasterization_diag sibling으로 재생했다. W155/W156/W157 Region lineage를 보존한 채 W162 frozen region_id=1 GLOBAL_OCCLUDED 6,197행과 161 camera를 exact stable-ID로 join했다. Region 0/1 population은 248,086/65,471이고 stable-ID overlap은 0이다.
- 총 997,717 query-camera pair에서 exact contributor 1,724, complete-prefix NOT_CONTRIBUTOR 970, truncation으로 인한 PROVENANCE_UNAVAILABLE 995,023을 기록했다. median representative는 same Gaussian 2, same Region different Gaussian 904,400, different Region 93,315이다. exact conflict 1,724 pair가 존재하지만 unavailable population도 크므로 architecture result는 MIXED이며, conflict는 physical false positive가 아닌 mechanical coexistence로 기록했다.
- canonical renderer와 diagnostic sibling의 RGB/alpha/median depth bitwise equivalence를 frozen review camera 3개에서 확인했다. control은 tabletop/table-side/vase/background를 포함하고, output은 27 PNG/0 PPM, 모든 visualization directory별 UTF-8 README, camera_name_stem.png direct layout이다. per-pixel per-primitive alpha*T는 unavailable이며 W161 spatial-domain work는 계속 paused다.

## 2026-09-04 Worklog 164 Canonical Renderer-Contributor Primitive Observability / Historical Candidate-B Controlled A/B

- [Worklog 164](worklogs/164_canonical_renderer_contributor_primitive_observability_historical_candidate_b_controlled_ab.md)는 W163의 `K=16` prefix를 participation 판정에 사용하지 않고, 기존 canonical acceptance path의 per-primitive `forward_accepted[g,v]` bit로 `CONTRIBUTED_IN_CAMERA`를 exact 수집했다. `POINT_QUERY_STATE`와 `PRIMITIVE_OBSERVATION_STATE`를 분리하고, 후자에만 positive `OBSERVED` override를 적용했다.
...
## 2026-09-06 Worklog 168 Raw Zero-Set First-Hit Positive Per-View Occlusion Evidence Audit

- [Worklog 168](worklogs/168_raw_zero_set_first_hit_positive_per_view_occlusion_evidence_audit.md)은 unfiltered historical raw zero-set first hit `z_s`와 analytic first blocker `z*`의 complete query-depth consequence를 strict no-epsilon contract로 비교했다. W167 single-surface fixture count를 exact 재현하고 two-sheet layered fixture를 추가했으며 geometry, component, `h/mu`, iso-value, Candidate-B를 변경하지 않았다.
...
## 2026-09-06 Worklog 170 Construction-Native Conservative Blocker Certificate Audit

- [Worklog 170](worklogs/170_construction_native_conservative_blocker_certificate_audit.md)은 W168 네 fixture의 raw zero-set first hit `9,428`개에서 owning cell, eight-corner TSDF value/`support_count`, marching-cubes interpolation 및 ray/cell extent를 전수 감사했다. 모든 hit cell은 eight-corner authoritative/sign-changing이었고 W168/W169 artifact hash는 실행 전후 동일했다.
- `SparseProjectiveTSDF`는 fused mean `value`와 contributor 수만 저장하며 per-view value, source-camera identity, projective-depth extrema를 보존하지 않는다. `h`, `mu`, cell diagonal/ray chord, iso-value 및 W169 empirical error도 construction semantics상 physical surface의 one-sided bound가 아니다.
- 최종 verdict는 `NO_CONSTRUCTION_NATIVE_BLOCKER_CERTIFICATE`다. `BEHIND_ZEROSET`은 representation-level geometry fact로만 유지하며 `CERTIFIED_BLOCKED`를 정의하지 않았다. epsilon/margin/filtering/global state 변경 없이 W161 pause를 보존했고 focused tests는 `4 passed`다.
