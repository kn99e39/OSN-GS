...
## 2026-08-19 Coverage-first Gaussian Subset partition (신규 top-level architecture 1단계)

- [Worklog 105](worklogs/105_coverage_first_gaussian_subset_partition.md)는 Worklog 95~104의 selection-first 구조(구조/support predicate를 통과한 evidence만 component·chart·patch가 되는 구조)를 계속 개선하지 않고, top-level visible-surface construction을 **coverage-first**로 교체하는 첫 구현 배치다: 전체 학습된 visible Gaussian scene → Gaussian마다의 surface-orientation 표현 → coverage 보존 normal-coherent·spatially-connected Gaussian Subset 분할. Trustable Gaussian 추정·latent surface·NURBS는 이번 배치에서 구현하지 않았고, Worklog 95~104 구현·산출물은 그대로 보존된다.
- 신규 `osn_gs/surface/torch_gaussian_surface_orientation.py`는 3DGS parameterization에서 주축을 **정확히** 뽑는다(`Sigma = R diag(exp(scaling)^2) R^T`이므로 `R`의 열이 곧 고유벡터 — eigen-decomposition 불필요). 유일한 모호성인 **축 순서**를 고윳값 내림차순으로 해소하고 `normal=axis(lambda3)`, `tangent_u=axis(lambda1)`, `tangent_v=normal x tangent_u`로 정의한다. 부호는 재현성 게이지일 뿐이고, 유사도 비교는 항상 unsigned `|dot(n_i,n_j)|`다.
- 신규 `osn_gs/surface/torch_coverage_first_subset_partition.py`는 kNN spatial adjacency(local median spacing 기준 scale-aware) 위에서만 normal compatibility를 평가해(전역 normal clustering 아님) 연결 성분을 만든다. **모든 heuristic은 `CoverageFirstPartitionConfig` 한 곳에 모여 있고 report에 그대로 기록된다**: `neighbor_count=8`, `spatial_connect_spacing_multiplier=2.0`, `normal_compatibility_min_alignment=0.85`(31.79°) — 앞의 둘은 기존 저장소 기본값 재사용, 렌더를 보고 조정한 값은 없다.
