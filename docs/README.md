...
- Current governing methodology: [OSN_GS_Urgent_Work_Master.md](Urgent_Work/OSN_GS_Urgent_Work_Master.md) (top-level, boundary-conditioned occluded-surface construction) with the active gate in [OSN_GS_Urgent_Work_Master.md](Urgent_Work/OSN_GS_Urgent_Work_Master.md). Phases A–E are implemented and Gate A–E approved; Phase F is next.
...
- `docs/Urgent_Work/OSN_GS_Urgent_Work_Master.md`를 기존 Phase 1–5 문서보다 상위의 methodology 재정의 문서로 검토했다. 새 방향은 global component recovery를 선행 성공조건에서 내리고, local visible NURBS boundary reconciliation → continuation domain → bounded multi-sided occluded candidate → constrained NURBS → uncertainty/validation 순서로 전환한다.
...
- Production 코드와 기본값은 변경하지 않았다. 감사 결과는 `docs/worklogs/74_direction_reset_interface_audit.md`, 승인 전용 구현 초안은 `docs/Urgent_Work/OSN_GS_Urgent_Work_Master.md`에 기록했다. 최초 승인 요청 범위는 Phase A–B data contract와 isolated artificial-boundary reconciliation prototype뿐이다.
...
- 전체 unittest `221 passed, 1 skipped`, pytest `220 passed, 1 skipped, 8 subtests passed`. Known `curved_annulus`/`mild_curved_sheet` blocker와 production component membership은 그대로다. Phase C 이후는 미승인 상태이며 Gate B에서 멈춘다. 상세 근거는 `docs/worklogs/75_boundary_conditioned_phase_ab.md`, active gate는 `docs/Urgent_Work/OSN_GS_Urgent_Work_Master.md`를 참고한다.
...
- Phase D continuation domain은 구현·Gate D 승인 완료다. canonical 계약은 `docs/Urgent_Work/OSN_GS_Urgent_Work_Master.md` §6, 구현 근거는 `docs/worklogs/81_phase_d_continuation_domain_implementation.md`를 따른다. Phase C evidence 실제 결합과 production integration은 여전히 별도 범위다.
...
- 2026-07-26: Phase G Gate 승인 후 model-only Uncertain Gaussian append adapter foundation을 구현했다. appearance/opacity unset은 명시적 blocker로 유지하고 optimizer/trainer/renderer/checkpoint integration은 시작하지 않았다. 상세: docs/worklogs/88_uncertain_gaussian_append_adapter_foundation.md.
- 2026-07-26: Urgent Work의 현행 방향, 상위 문서 관계, deprecated research branch, append adapter Gate 범위를 docs/Urgent_Work/OSN_GS_Urgent_Work_Master.md로 통합했다. 기존 Urgent Work 문서는 제거하며, 이력은 Worklog와 Git에 보존한다.

- 2026-07-27: 모든 topology에 공통인 Boundary-first support-curve foundation을 별도 모듈로 추가했다. 기존 constructor dispatcher에는 연결하지 않았으며, 결과와 integration 범위는 docs/worklogs/89_boundary_first_support_curve_foundation.md에 기록했다.

- 2026-07-27: Boundary-first pre-surface visible builder가 observed outer/hole loop에서 support-curve seam multi-patch를 만들도록 추가했다. fallback 대신 unsupported를 반환하며, 현재 국소 재구축은 약 70%다. 상세: docs/worklogs/90_boundary_first_pre_surface_visible_builder.md.

- 2026-07-27: isolated curved_annulus Boundary-first proof path을 완료했다. split component recovery → observed inner/outer loop → support-curve seam multi-patch를 검증했으며, 기존 dispatcher는 변경하지 않았다. 상세: docs/worklogs/91_boundary_first_curved_annulus_recovery_proof.md.

- 2026-07-27: Boundary-first routing에 false-hole negative-control gate를 추가했다. curved_annulus는 통과하고 plane/parallel sheet의 noise hole은 unsupported로 차단한다. integration 준비는 약 35%이며 상세는 docs/worklogs/92_boundary_first_negative_control_routing_gate.md에 있다.
- 2026-07-27: Boundary-first recovery sweep을 point count 400/600 및 seed 0/1/2에서 검증했다. curved_annulus는 모든 조합에서 복구되고 close_parallel_sheets는 모두 거부된다. integration 준비는 약 50%이며 상세는 docs/worklogs/93_boundary_first_recovery_sweep_gate.md에 있다.
- 2026-07-27: Boundary-first materialized patch의 support sample·cyclic seam·corner Jacobian 수치 게이트를 추가했다. 이는 원본 경계 전체 fidelity와 구분되는 진단이며, integration 준비는 약 55%다. 상세는 docs/worklogs/94_boundary_first_surface_quality_gate.md에 있다.

- 2026-07-27: 격리 Boundary-first review runner가 전체 synthetic scene의 constructed/unsupported 상태와 renderer artifact를 export한다. 15개 scene 중 6개가 현재 paired-boundary 경로로 구성되고 9개는 누락된 interior support 또는 multi-loop pairing 사유를 기록한다. integration 준비는 약 65%이며 상세는 docs/worklogs/95_boundary_first_all_scene_review_export.md에 있다.

- 2026-07-27: Boundary-first의 다음 일반화는 annulus 확장이 아니라 모든 topology에 공통인 ordered observed boundary loop와 provenance 있는 interior support/central-cap 계약이다. 임의 축소 loop나 rectangle fallback은 사용하지 않는다. 상세는 docs/worklogs/97_boundary_first_universal_interior_support_policy.md에 있다.

- 2026-07-27: Boundary-first는 ordered loop·observed-anchor central cap·source RMS review gate까지 격리 구현됐다. feature-gated 15-scene review는 constructed 8, review_required 1, unsupported 6이며 상세는 docs/worklogs/98_boundary_first_ordered_loop_and_central_cap_foundation.md, docs/worklogs/99_boundary_first_source_fidelity_review.md, docs/worklogs/100_boundary_first_feature_gated_review.md에 있다.

- 2026-07-27: Boundary-first visible builder는 topology별 다른 방법론 대신 `outer_boundary + interior_boundary/observed interior_anchor` 역할 계약으로 통일했다. sine의 작은 KDE false hole은 support validation에서만 제한적으로 복원하되 진단 provenance에는 남긴다. 기본 dispatcher와 production은 변경하지 않았고, 관련 회귀 21건이 통과했다. 현재 국소 구현 진행률은 약 68%이며 상세는 `docs/worklogs/102_boundary_first_role_contract_and_false_hole_gate.md`를 참조한다.
- 2026-07-27: Boundary-first provenance에 source-boundary fidelity(원본 점군까지의 거리와 local-spacing 정규화)를 추가했다. 이는 자동 eligibility gate가 아닌 review metric이며, 새 15-scene artifact는 constructed 12 / review_required 1 / unsupported 2다. 상세: `docs/worklogs/103_boundary_first_source_boundary_fidelity_payload.md`.
- 2026-07-27: isolated Boundary-first support raster의 기본 resolution을 96으로 올리고, rasterization 양자화만 허용하는 1-cell anchor-ray tolerance를 provenance에 기록했다. sine/curved_annulus boundary fidelity는 각각 2.826→2.379, 2.491→2.113으로 개선됐으며 U-shape는 계속 거부된다. 상세: `docs/worklogs/104_boundary_first_resolution96_raster_tolerance.md`.
- 2026-07-27: isolated Boundary-first 변경 뒤 전체 pytest는 `475 passed, 3 failed, 1 skipped, 1 warning, 8 subtests passed`다. 실패 3건은 기존 annulus independent-fit guard 및 trimmed fitter degenerate-fraction 회귀이며 이번 변경 범위 밖이다. 상세: `docs/worklogs/105_boundary_first_full_regression_checkpoint.md`.
- 2026-07-27: resolution 96/1-cell tolerance sweep에서 sine·curved_annulus는 400/600 points와 seed 0/1/2의 각 6개 조합 모두 constructed, U-shape는 6개 조합 모두 unsupported였다. 상세: `docs/worklogs/106_boundary_first_resolution_tolerance_sweep.md`.
- 2026-07-27: multi-loop은 outer boundary를 hole마다 복제하지 않고, 공통 boundary-role evidence와 `non_overlapping_planar_domain_partition` 부족 사유를 review payload로 기록한다. 상세: `docs/worklogs/107_boundary_first_multi_loop_role_evidence.md`.
- 2026-07-27: multi-loop role evidence 이후 전체 pytest는 `478 passed, 3 failed, 1 skipped, 1 warning, 8 subtests passed`다. 기존 3개 공용 실패만 유지됐고 Boundary-first 증가분은 통과했다. 상세: `docs/worklogs/108_boundary_first_multi_loop_full_regression_checkpoint.md`.
- 2026-07-27: multi-loop materialization 전 planar partition evidence foundation을 추가했다. ordered/nesting/orientation/outer-owner 불변식을 검증하되 실제 chart는 생성하지 않는다. 상세: docs/worklogs/109_boundary_first_planar_partition_evidence.md.
- 2026-07-27: Boundary-first review payload의 control polygon과 실제 evaluated NURBS curve를 명시적으로 분리했고(`torch_boundary_review_geometry.py`), observed-anchor fan의 pole 스포크를 correspondence chord로 재분류했으며, 실제 evaluated support curve 사이 crossing gate를 추가했다. 이 gate가 plane/sine/crease/triangle/elongated_plane/close_parallel_sheets에서 이전까지 조용히 `constructed`로 export되던 실제 support-curve 겹침 결함을 처음으로 검출했다(근본 수정은 범위 밖). 전체 pytest는 `495 passed, 2 failed, 1 skipped`(실패 2건은 기존 trimmed-fitter 이슈로 무관). 상세: `docs/worklogs/110_boundary_first_review_geometry_semantics_and_crossing_gate.md`.
