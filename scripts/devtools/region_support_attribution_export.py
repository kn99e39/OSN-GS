"""Worklog 101 -- rejected-interface attribution + support-starvation
measurement (Worklog 100 baseline, FIXED_MASKED_KNN mode).

Runs `torch_region_adaptive_support_merge.partition_surfels_region_adaptive_support`
in `SUPPORT_MODE_FIXED_MASKED_KNN` (verified elsewhere to reproduce Worklog
100 exactly) on the same checkpoint as Worklog 96-100, and reports the full
rejected-interface attribution and region-size-conditioned support
statistics the architecture directive requires, WITHOUT yet running the
...
def _progress(message: str) -> None:
    print(f"[support attribution] {message}", flush=True)
...
def main() -> None:
    ...
    with torch.no_grad():
        ...
        config = AdaptiveSupportConfig(local=local_config, region=region_config, support_mode=SUPPORT_MODE_FIXED_MASKED_KNN)
    ...
    output_path = REPO_ROOT / "output/osn_gs_region_support_attribution/fixed_masked_knn_attribution.json"
